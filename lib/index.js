...
var funProcessDirectPayment = function funProcessDirectPayment(BASE_URL, payload, callback) {
  ...
  var cardType = '';
  if (!isNull(payload["meta"]["cardType"])) {
    cardType = payload["meta"]["cardType"];
  }
  var entryMode = '';
  if (!isNull(payload["meta"]["entryMode"])) {
    entryMode = payload["meta"]["entryMode"];
  }

  var url = BASE_URL + 'ezpayPaymentTransactions/processDirectPayment?payerId=' + payerId + '&transactionId=' + transactionId + '&cardId=' + cardId + '&hostBaseURL=' + hostBaseURL + '&entryMode=' +entryMode + '&cardType=' + cardType;
  axios.post(url, payload).then(function (response) {
    return callback(response);
  }).catch(function (error) {
    var json = stringify(error);
